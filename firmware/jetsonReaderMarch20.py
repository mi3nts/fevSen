...
def main():
    ...
    while(True):
        try:
            dateTime1    = datetime.datetime.now()+ lag
            dateTime2    = dateTime1 + add
            
            rawName1     = directory + getImagePathTailHdf5Mod2(dateTime1,'raw')
            rawName2     = directory + getImagePathTailHdf5Mod2(dateTime2,'raw')

            valName      = directory + getImagePathTailHdf5Mod2(dateTime1,'val')
    
            raw1Valid,left1,right1,thermal   = hdf5Reader(rawName1)
            raw2Valid,left,right,thermal2    = hdf5Reader(rawName2)
